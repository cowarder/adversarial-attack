    ...
    def generate(self, original_image, im_label, target_class):
        ...
        for i in range(10):
            ...
            if pred_index == target_class:
                ...
                cv2.imwrite('./generated/targeted_adv_noise_from_' + str(im_label) + '_to_' +
                            str(pred_index) + '.jpg', noise_image)
                # write image
                cv2.imwrite('./generated/targeted_adv_img_from_' + str(im_label) + '_to_' +
...
if __name__ == '__main__':
    target_example = 0  # choose image to attack
    (original_image, prep_img, img_label, _, pretrained_model) =\
        get_params(target_example)
    FGS_targeted = FastGradientSignTargeted(pretrained_model, 0.01)

    target_class = 24   # attack target
    FGS_targeted.generate(original_image, img_label, target_class)